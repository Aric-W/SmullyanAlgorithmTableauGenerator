...
What user input should look like:
    (1.) All valid input only contains the symbols '(',')','~','|','&','a','b','c','d','e','f','g',
...
A note on truth-functions:
    ...
    in-between conjoined formulas; they are:  '&' which represents logical conjunction, '|' which represents logical disjunction
    ...
    p&q is true, if both p and q are false, p|q is false, if p is true and q is false then 'p implies q' (p>q) is false.
    ...
    (p|~p) is true even when p is false! We can use truth-tables to determine whether or not a formula is a tautology, but a more
